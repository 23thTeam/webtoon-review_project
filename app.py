...
def webtoonDetail(webtoon_id):
    ...
    reviews = Review.query.filter_by(webtoon_id=webtoon_id)
    ...
    for review in reviews:
        webtoon_review_list.append(review)
    # review1 = {
    #     'username': '유저 이름',
    #     'title': '리뷰 제목',
...
@app.route("/webtoon/review_delete/")
def review_delete():
    id_receive = request.args.get("webtoon_id")

    data = db.session.query(Review).filter_by(webtoon_id=id_receive).first()
    db.session.delete(data)
    db.session.commit()

    webtoon_list = Review.query.all()
    return render_template('home.html', data=webtoon_list)
